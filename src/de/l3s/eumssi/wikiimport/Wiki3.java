...
public class Wiki3 {
    ...
    public void run(){
        ...
        for(int i=10;i<=60;i++){
            ...
            String url = "http://en.wikipedia.org/wiki/"+returnMonth(i-monthoffset)+"_"+(2000+yearoffset);
            if(i%12 == 0)
            {
                check = true;
            }
            ...
            for(Element eachday : days){  // This will loop only once because it is the whole text
                ...
                for(Element dateplustext : individual){    // This consists of alternate date and events (with or withour newsStory)
                    if(dateplustext.tagName().equals("h3")){ // Upto December 2004
                        ...
                        if(!modifiedDate.toLowerCase().contains("200".toLowerCase()) && i!=48 && i!=49){
                            continue;
                        }
                        if((2000+yearoffset)>= 2003 && i>=40){
                            if(da<=9)
                                actualDate = (2000+yearoffset)+"-"+mont+"-0"+da;
                            else
                                actualDate = (2000+yearoffset)+"-"+mont+"-"+da;
                            da++;
                        }
                        else{
                            if(modifiedDate.toLowerCase().contains(",".toLowerCase())){
                                ...
                                actualDate = year+"-"+monthvalue(returnMonth(i-monthoffset))+"-"+day;
                            }
                        }
                        print("______DATE : %s\n",actualDate);
                    }
                                        ...
                                        if (ul.tagName().equals("ul")) {
                                            ...
                                            for(Element inside : innerli){    // Here we are picking one li
                                                ...
                                                try{
                                                	...
                                                	System.err.println("actual date: " + actualDate);
                                                	e.printStackTrace();
                                                }
                                            }
